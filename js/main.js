$(document).ready(() => {
  const $charCount = $("#tweet-modal .char-count");
  const maxCharCount = parseInt($charCount.data("max"), 10);

  $("#tweet-modal textarea").on("keyup", e => {
    const tweetLength = $(e.currentTarget).val().length;
